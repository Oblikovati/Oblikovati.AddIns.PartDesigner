# Build the oblikovati-part-designer add-in as a c-shared library (.so/.dll/.dylib) and
...
LIB := oblikovati-part-designer.$(EXT)
OUT ?= build
...
.PHONY: build install test clean sync-header
...
test: sync-header ## Run the add-in tests (cgo-free designer + full-stack E2E)
	go test ./...
