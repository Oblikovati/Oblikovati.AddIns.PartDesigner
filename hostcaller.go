...
// cgoHostCaller implements designer.HostCaller over the C-ABI host callbacks stored on
// Activate. Call blocks until the host runs the request on its session goroutine.
type cgoHostCaller struct{}

// Call forwards a JSON method request to the host and returns its JSON reply. A non-OK
// status returns the host's error message; the response buffer (host-owned) is always
// released via ObkHostFree.
func (cgoHostCaller) Call(method string, req []byte) ([]byte, error) {
	mu.Lock()
	call, freeFn := hostCall, hostFree
	mu.Unlock()
	if call == nil {
		return nil, errors.New("part-designer: host not connected")
	}
	...
	if resp == nil {
		...
		return nil, errors.New("part-designer: host call failed")
	}
