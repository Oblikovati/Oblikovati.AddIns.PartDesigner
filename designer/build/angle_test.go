...
func angleMember(designation string, legA, legB, thickness, length float64) ResolvedMember {
	fam := &catalog.Family{
		...
		Members: []catalog.Member{{
			...
			Values: map[string]float64{"a": legA, "b": legB, "t": thickness, "l": length},
...
// TestAngleBuildsHeelAnchoredSection is the C3 angle acceptance check: the three section
// parameters are published, the 6-vertex L outline is axis-aligned, the heel is fixed at the
// origin, the legs and thicknesses are sized by distance dimensions, and it extrudes to `length`.
func TestAngleBuildsHeelAnchoredSection(t *testing.T) {
	h := &fakeHost{dof: 0}
	if err := (Angle{}).Build(newBuilder(h, catalog.UnitsMillimetre),
		angleMember("L 60x40x5", 60, 40, 5, 6000)); err != nil {
	...
	// Heel fixed at the origin; the legs and thicknesses are distance-driven.
	if got := countKind(h.constraints, "fix"); got != 1 {
		t.Errorf("fix count = %d, want 1 (the heel)", got)
	}
	if got := countKind(h.constraints, "horizontal") + countKind(h.constraints, "vertical"); got != 6 {
		t.Errorf("axis alignments = %d, want 6 (L-section)", got)
	}
	for _, expr := range []string{"leg_a", "leg_b", "thickness"} {
		if !hasDimension(h.dimensions, expr) {
			t.Errorf("distance dimension %q not applied; have %+v", expr, h.dimensions)
		}
	}
}
